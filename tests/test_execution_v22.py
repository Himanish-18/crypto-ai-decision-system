...
class TestExecutionV22(unittest.TestCase):
    ...
    def test_fill_prob(self):
        ...
        prob_adverse = self.fill_model.predict(
            "BUY", distance_pct=0.0, obi=0.8
        )  # Strong buy pressure
        # prob = 1 / (1 + exp -(0.5 + 0 + 1.0*(-0.8))) = sigmoid(0.5 - 0.8) = sigmoid(-0.3) < 0.5

        prob_favorable = self.fill_model.predict(
            "BUY", distance_pct=0.0, obi=-0.8
        )  # Strong sell pressure (price drops to us)
    ...
    def test_simulator(self):
        ...
        self.assertEqual(
            res["fills"][0]["fill_price"], 100.0
        )  # Limit filled at limit price?
