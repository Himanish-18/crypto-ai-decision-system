...
def setup_engine():
    ...
    with patch.object(LiveSignalEngine, 'load_artifacts'), \
         patch.object(LiveSignalEngine, 'load_hybrid_models'):
...
def run_fuzz_test(engine, n=100):
    ...
    for i in range(n):
        ...
        if chaos_type == 0:
            ...
            if "btc_volume" in df.columns: df = df.drop(columns=["btc_volume"])
            desc = "Missing Volume"
        ...
        try:
            ...
            if not all(k in res for k in required):
                logger.error(f"❌ Malformed output for {desc}: Missing keys {set(required) - set(res.keys())}")
                crashes += 1
        except Exception as e:
