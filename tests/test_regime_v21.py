...
class TestRegimeV21(unittest.TestCase):
    ...
    def test_hard_veto(self):
        ...
        hist_safe = pd.DataFrame({
            "close": np.random.normal(50000, 10, 100) # Low vol
        })
        
        # Mock macro feeds to return safe
        brain.macro_feeds.fetch_live_metrics = lambda: {"funding_rate": 0.0001, "iv_index": 30}
        
        # Pass dict as expected by base classes
        decision = brain.think({"candles": hist_safe, "regime": "NEUTRAL", "action": "HOLD"}) 
        ...
        brain.macro_feeds.fetch_live_metrics = lambda: {"funding_rate": -0.005, "iv_index": 90}
