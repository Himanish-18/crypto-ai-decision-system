...
def run_simulation():
    ...
    sim_data["atr_pct"] = sim_data["btc_atr_14"].rank(
        pct=True
    )  # Approximation for this snippet
    ...
    for i in range(len(sim_data)):
        ...
        if decision["action"] == "BUY":
            buy_signals += 1
            logger.info(
                f"[{decision['timestamp']}] BUY SIGNAL! Size: {decision['size']:.4f} BTC"
            )
        elif i < 10 or i % 500 == 0:
            logger.info(
                f"[{decision['timestamp']}] SKIP: {decision['reason']} | Prob: {signal_output['prediction_prob']:.4f} | RSI: {signal_output['strategy_context']['rsi']:.1f}"
            )
