...
def run_sim():
    ...
    metrics = engine.calculate_risk_metrics(
        pd.DataFrame({"BTC": np.random.normal(0, 0.02, 100)})
    )
    ...
    if trades:
        for t in trades:
            print(
                f"    🚨 PROPOSAL: {t['side']} {t['amount_usd']:,.2f} {t['instrument']} | Reason: {t['reason']}"
            )
    else:
    ...
    # Recalc
    new_metrics = engine.calculate_risk_metrics(
        pd.DataFrame({"BTC": np.random.normal(0, 0.02, 100)})
    )
