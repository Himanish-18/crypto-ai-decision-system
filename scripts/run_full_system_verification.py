...
def run_test(command, version_name):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return {"version": version_name, "status": "PASS", "duration": duration, "output": result.stdout}
        else:
            logger.error(f"❌ {version_name} FAILED ({duration:.2f}s)")
            logger.error(result.stderr)
            return {"version": version_name, "status": "FAIL", "duration": duration, "error": result.stderr}
...
def main():
    ...
    results.append(run_test("python3 tests/quantum_testbench.py", "v19_Quantum_Stacker"))
    ...
    # v22: Execution Logic
    results.append(run_test("python3 tests/test_execution_v22.py", "v22_Execution_Logic"))
    
    # v23: Portfolio Opt
    results.append(run_test("python3 tests/test_portfolio_env.py", "v23_Portfolio_RL_Unit"))
    results.append(run_test("bash scripts/backtest_portfolio.sh", "v23_Portfolio_RL_Backtest"))
    ...
    for r in results:
        ...
        print(f"{status_icon} {r['version']}: {r['status']} ({r.get('duration',0):.2f}s)")
        if r["status"] != "PASS":
            all_pass = False
