...
def main():
    ...
    for i in range(args.steps):
        ...
        if i % 10 == 0:
            logger.info(f"Step {i}: Val={current_val:.2f}, Reward={reward:.4f}, Turnover={info['turnover']:.4f}")
    ...
    logger.info(f"Avg Turnover: {np.mean([np.sum(np.abs(w - weights_history[max(0, i-1)])) for i, w in enumerate(weights_history)]):.4f}")
