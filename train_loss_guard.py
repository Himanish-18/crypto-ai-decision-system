...
def feature_engineering(df):
    ...
    if "fundingRate" in df.columns:
        ...
        df["funding_flip"] = (df["funding_sign"] != df["funding_sign"].shift(1)).astype(
            int
        )
    else:
        df["funding_flip"] = 0
...
def train_model():
    ...
    if len(df[TARGET_COL].unique()) < 2:
        logger.error(
            "Data does not contain both Safe and Loss examples. Cannot train classifier."
        )
    ...
    rec = recall_score(
        y_test, y_pred
    )  # Recall is important for Safety (catching losses)
    prec = precision_score(y_test, y_pred)

    logger.info(
        f"Model Results -> Acc: {acc:.4f} | AUC: {roc:.4f} | Recall (Safety): {rec:.4f} | Precision: {prec:.4f}"
    )

    if rec < 0.1:
        logger.warning(
            "Recall is very low. Model might not be catching crashes effectively. Adjust threshold or weights."
        )
