...
async def test_ob_manager():
    ...
    class MockBinanceExecutor:
        def place_order(self, symbol, side, amount, order_type, price=None):
            return {"id": "mock_id", "status": "filled", "type": order_type, "price": price}
    ...
    res1 = await smart_exec.execute_order("BTC/USDT", "buy", 0.1, style="AUTO", microstructure=metrics_agg)
    ...
    res2 = await smart_exec.execute_order("BTC/USDT", "buy", 0.1, style="AUTO", microstructure=metrics_pas)
    ...
    metrics_panic = {"obi": 0.1, "spread_pct": 0.0001, "impact_cost": 0.002} # > 0.15% (0.0015)
    logger.info("Test Case 3: High Impact Cost")
    res3 = await smart_exec.execute_order("BTC/USDT", "buy", 0.1, style="AUTO", microstructure=metrics_panic)
