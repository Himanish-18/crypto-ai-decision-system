...
class StressAudit:
    ...
    def evaluate_model(self, df_subset, name):
        ...
        exclude_cols = ["timestamp", target_col, "btc_ret_fwd_1", "y_pred", "y_prob", "signal_prob", "regime", "symbol", "mf_score", "entry_signal", "dyn_sl", "dyn_tp", "dyn_pos_size"]
        ...
        try:
            ...
            res = {"auc": 0.5, "precision": 0, "recall": 0, "f1": 0, "count": len(df_subset)}
    ...
    def run_data_stress_tests(self):
        ...
        if "btc_atr_14" in self.df.columns:
            ...
            results["High Volatility"] = self.evaluate_model(high_vol_df, "High Volatility")
    ...
    def run_strategy_simulations(self):
        ...
        opt = StrategyOptimizer(MODEL_PATH, SCALER_PATH, FEATURES_FILE, fee_rate=base_fee, slippage=base_slip)
        opt.load_artifacts()
        opt.prepare_data()
        opt.run_strategy()
        ...
        opt = StrategyOptimizer(MODEL_PATH, SCALER_PATH, FEATURES_FILE, fee_rate=base_fee, slippage=base_slip*2)
        opt.load_artifacts()
        opt.prepare_data()
        opt.run_strategy()
        ...
        opt = StrategyOptimizer(MODEL_PATH, SCALER_PATH, FEATURES_FILE, fee_rate=base_fee*3, slippage=base_slip)
        opt.load_artifacts()
        opt.prepare_data()
        opt.run_strategy()
        ...
        opt = StrategyOptimizer(MODEL_PATH, SCALER_PATH, FEATURES_FILE, fee_rate=base_fee, slippage=0.01)
        opt.load_artifacts()
        opt.prepare_data()
        opt.run_strategy()
    ...
    def run_risk_tests(self):
        ...
        re_results["Sizing_Reduction"] = "PASS" if val_5k < val_10k * 0.6 else "FAIL" # Expect ~50%
    ...
    def generate_report(self):
        ...
        base_auc = self.metrics.get("data_stress", {}).get("High Volatility", {}).get("auc", 0.5) # Proxy
        # Strategy Score: Base Sharpe/ProfitFactor
        base_pf = self.metrics.get("strategy_stress", {}).get("Base Case", {}).get("profit_factor", 1.0)
        
        self.scores["Model Score"] = min(100, max(0, base_auc * 100)) # Simple
        self.scores["Strategy Score"] = min(100, max(0, base_pf * 30)) # PF 3.0 = 90
        self.scores["Risk Score"] = 100 if all(v == "PASS" for v in self.metrics["risk_stress"].values()) else 50
        self.scores["Readiness"] = (self.scores["Model Score"] + self.scores["Strategy Score"] + self.scores["Risk Score"]) / 3
...
"""
        ...
        report += """
## 5. Recommendations
- **High Volatility**: """ + ("Model degrades." if self.scores["Model Score"] < 60 else "Model holds up.") + """
- **Slippage Sensitivity**: """ + ("Critical." if self.metrics["strategy_stress"]["2x Slippage"]["profit_factor"] < 1.1 else "Managed.") + """
