...
def main():
    ...
    if "crisis" in model.models and "xgb" in model.models["crisis"].models:
        ...
        if hasattr(xgb_model, "feature_names_in_"):
            ...
            imp_df = pd.DataFrame({"Feature": feats, "Importance": imps}).sort_values(
    ...
    with open(REPORT_PATH, "w") as f:
        ...
        f.write(
            "The Multi-Factor Alpha Model was upgraded to include regime-specific sub-models (Normal vs Crisis) "
        )
        f.write(
            "and enhanced with 6+ new features targeting correlation, momentum, and microstructure dynamics. "
        )
        f.write(
            "Training was performed using robust HistGradientBoosting to handle data irregularities.\n\n"
        )
        ...
        for r, res in metrics.items():
            ...
            icon = (
                "EXCELLENT"
                if change > 0.05
                else ("IMPROVED" if change > 0.0 else "NEUTRAL")
            )
            if change < 0:
                icon = "REGRESSED"

            f.write(
                f"| {r} | {before:.2f} | **{after:.4f}** | {change:+.4f} ({icon}) |\n"
            )
