...
def audit_pipeline_integrity():
    ...
    try:
        ...
        required_cols = [
            "btc_alpha_trade_imbalance",
            "btc_alpha_vwap_reversal",
...
def audit_strategy_performance():
    ...
    try:
        ...
        config = BacktestConfig(
            taker_fee=0.0004, maker_fee=0.0002, use_regime_filter=True
        )
...
def audit_rl_ensemble():
    ...
    try:
        config = BacktestConfig(
            taker_fee=0.0004, use_regime_filter=True, use_rl_ensemble=True  # Enable RL
        )
...
def run_audit():
    ...
    for k, v in results.items():
        status = "✅ PASS"
        if v == False or v == "FAIL":
            status = "❌ FAIL"
        elif v == "WARN":
            status = "⚠️ WARN"
        elif isinstance(v, dict):
            status = f"✅ PASS (PF: {v.get('profit_factor', 0):.2f})"
