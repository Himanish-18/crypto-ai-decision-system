...
def run_diagnostic():
    ...
    logger.info(f"Targeting {len(of_features)} Order Flow and {len(sent_features)} Sentiment features.")
    ...
    for regime in regimes:
        ...
        if regime == "Normal":
            # Exclude crisis
            subset = df[~df["regime"].isin(["High Volatility", "Low Liquidity", "Macro Event"])].copy()
        else:
        ...
        # Train Proxy
        exclude = ["timestamp", "y_direction_up", "btc_ret_fwd_1", "regime", "is_shock", "is_uptrend", "symbol"]
        features = [c for c in subset.columns if c not in exclude and pd.api.types.is_numeric_dtype(subset[c])]
