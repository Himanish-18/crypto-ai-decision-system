...
class ProbabilityCalibration:
    ...
    def calibrate(self, base_model: BaseEstimator, X_val: np.ndarray, y_val: np.ndarray) -> BaseEstimator:
        """
        ...
        try:
            calibrated_model = CalibratedClassifierCV(estimator=base_model, method=self.method, cv='prefit')
        except TypeError:
            calibrated_model = CalibratedClassifierCV(base_estimator=base_model, method=self.method, cv='prefit')
