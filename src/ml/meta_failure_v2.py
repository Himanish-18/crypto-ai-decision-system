...
class MetaFailureModel:
    ...
    def train(self, trades_df: pd.DataFrame):
        ...
        required_features = ['volatility', 'spread', 'imbalance', 'regime_score', 'model_confidence']
        ...
        if y.sum() == 0:
            logger.warning("No failures in dataset (Good job?). Cannot train failure model.")
            return
            
        self.model.fit(X, y)
        self.is_fitted = True
        
        logger.info(f"Meta-Failure Model Trained. OOB Score: {self.model.score(X, y):.2f}")
        logger.info(f"Feature Importance: {dict(zip(available_features, self.model.feature_importances_))}")
    ...
    def should_veto(self, market_state: Dict, veto_threshold: float = 0.6) -> bool:
        ...
        if p_fail > veto_threshold:
            logger.warning(f"Meta-Failure VETO: Prob Failure {p_fail:.2f} > {veto_threshold}")
            return True
        return False
