...
class ResidualReturnModel:
    ...
    def fit_transform(self, asset_returns: pd.Series, market_returns: pd.Series, other_factors: pd.DataFrame = None) -> pd.Series:
        """
        ...
        logger.info(f"Residual Model Fit. Market Beta: {self.betas.get('Market', 0):.4f}")
    ...
    def extract_pca_residuals(self, returns_df: pd.DataFrame) -> pd.DataFrame:
        ...
        X_sys = self.pca_model.pca.inverse_transform(self.pca_model.pca.transform(returns_df))
        ...
        logger.info(f"PCA Removed {explained*100:.2f}% Systematic Variance. Remaining is Pure Alpha.")
