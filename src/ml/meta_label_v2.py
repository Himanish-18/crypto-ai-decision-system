...
class TripleBarrierLabeler:
    ...
    def __init__(
        self, profit_take: float = 0.02, stop_loss: float = 0.01, time_limit: int = 120
    ):
    ...
    def get_labels(
        self, close_prices: pd.Series, volatility: Optional[pd.Series] = None
    ) -> pd.Series:
        """
...
class LossProbabilityModelV2:
    ...
    def predict_loss_prob(
        self, market_data: Dict[str, Any], proposed_decision: Dict[str, Any]
    ) -> float:
        """
        ...
        if action == "BUY" and funding < -0.001:
            p_loss += 0.2
        if action == "SELL" and funding > 0.001:
            p_loss += 0.2
