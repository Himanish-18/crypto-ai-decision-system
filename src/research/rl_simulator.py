...
class CryptoEnv:
    def __init__(self, df: pd.DataFrame, initial_balance=10000):
        ...
        self.observation_space = (
            len(self.available_features) + len(self.l2_features) + 2
        )  # +2 for Position State
    ...
    def _get_obs(self):
        ...
        obs = self.df.iloc[self.current_step][self.available_features].values.astype(
            np.float32
        )
    ...
    def step(self, action):
        ...
        for order in self.pending_orders:
            ...
            if not filled and (
                self.current_step - created < 10
            ):  # Expire after 10 steps
...
def main():
    ...
    for e in range(episodes):
        ...
        logger.info(
            f"Episode {e+1}/{episodes} | Total Reward: {total_reward:.2f} | Epsilon: {agent.epsilon:.2f}"
        )
