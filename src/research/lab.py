...
class ResearchLab:
    ...
    def detect_drift(self, reference_data: pd.DataFrame, live_data: pd.DataFrame) -> Dict[str, float]:
        """
        Detect feature distribution shift using KSI (Kolmogorov-Smirnov) or PSI.
        """
        drift_report = {}
        for col in reference_data.columns:
            if col not in live_data.columns: continue
    ...
    def walk_forward_optimization(self, strategy_func, data: pd.DataFrame, window_size: int = 1000):
        """
