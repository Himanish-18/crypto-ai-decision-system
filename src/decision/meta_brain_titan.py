...
class MetaBrainHardVeto(MetaBrainV17):
    ...
    def think(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if regime in ["LIQ_CRUNCH", "VOL_SHOCK"]:
             logger.critical(f"🛑 TITAN HARD VETO ACTIVATED. Regime: {regime}. FORCING CASH.")
             return {
                 "action": "HOLD", 
                 "size": 0.0, 
                 "agent": "TITAN_VETO", 
        ...
        if p_loss > 0.60:
            logger.warning(f"🛡️ TITAN META-LABEL VETO. P(Loss) {p_loss:.2f} > 0.60. Blocking Agent {decision['agent']}.")
            return {
                "action": "HOLD",
                "size": 0.0,
