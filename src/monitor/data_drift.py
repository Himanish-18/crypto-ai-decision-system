...
class DriftDetector:
    ...
    def load_reference(self):
        ...
        self.numeric_features = [
            ...
            if c not in exclude
            and np.issubdtype(self.reference_data[c].dtype, np.number)
        ]
    ...
    def check_drift(self, live_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature in self.numeric_features:
            ...
            if status != "OK":
                logger.warning(
                    f"⚠️ Drift detected in {feature}: PSI={psi:.4f} ({status})"
                )
...
def main():
    ...
    if detector.reference_data is not None:
        ...
        if "regime" not in live_data.columns:
            ...
            live_data["regime"] = np.random.choice(
                ["Normal", "High Volatility"], size=len(live_data), p=[0.5, 0.5]
            )
