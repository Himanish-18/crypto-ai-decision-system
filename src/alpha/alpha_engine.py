...
class AlphaEngine:
    ...
    def _compute_on_chain_alphas(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["alpha_whale_flow"] = np.where(
            vol_z > 2, 
            np.sign(df["btc_close"] - df["btc_open"]), 
            0
        )
        # Decay the signal
        ...
        df["alpha_exchange_delta"] = -rsi_norm # High RSI -> Negative Alpha (Expect Reversal)
        ...
        df["alpha_miner_flow"] = macd_hist # Positive Momentum -> Positive Alpha (Miners holding)
    ...
    def _compute_microstructure_alphas(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["alpha_ofi"] = (2 * df["btc_close"] - df["btc_high"] - df["btc_low"]) / (df["btc_high"] - df["btc_low"] + 1e-9)
        ...
        df["alpha_vpin_proxy"] = -df["alpha_vpin_proxy"] # Higher is better (more liquid)
