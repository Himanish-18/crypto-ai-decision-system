...
class PortfolioRiskEngine:
    ...
    def calculate_risk_metrics(self, history: pd.DataFrame) -> Dict[str, float]:
        ...
        beta_weighted_exp = sum(
            d["usd_value"] * d["beta_mkt"] for d in self.positions.values()
        )
        ...
        return {
            ...
            "leverage": (
                sum(abs(d["usd_value"]) for d in self.positions.values()) / self.equity
                if self.equity > 0
                else 0
            ),
        }
    ...
    def check_constraints(
        ...
        if metrics.get("var_99", 0) > constraints.get("max_var", 1.0):
            violations.append(
                f"VaR Violation: {metrics['var_99']:.2%} > {constraints['max_var']:.2%}"
            )

        if metrics.get("leverage", 0) > constraints.get("max_leverage", 10.0):
            violations.append(
                f"Leverage Violation: {metrics['leverage']:.2f} > {constraints['max_leverage']}"
            )
