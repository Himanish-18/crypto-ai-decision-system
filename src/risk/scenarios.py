...
class ScenarioSimulator:
    ...
    def simulate(self, positions: Dict[str, Dict], scenario_name: str = None, custom_shock: Dict = None) -> float:
        """
        Calculate Estimated PnL of portfolio under shock.
        positions: Output from PortfolioRiskEngine (dict of asset details)
        """
        shocks = custom_shock if custom_shock else self.standard_scenarios.get(scenario_name, {})
        if not shocks:
            return 0.0
