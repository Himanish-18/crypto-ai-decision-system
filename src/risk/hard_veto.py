...
class HardVeto:
    ...
    def check_portfolio_health(self, nav: float, positions: Dict[str, float], metrics: Dict[str, float]) -> bool:
        """
        ...
        if not weights: return True # Empty portfolio is safe
        ...
        if len(fails) > (len(stress_df) * 0.5):
            logger.critical("🛑 HARD VETO: Portfolio too fragile (Failed >50% Stress Scenarios).")
            return False
