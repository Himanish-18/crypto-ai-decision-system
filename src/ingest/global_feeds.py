...
class GlobalMacroFeeds:
    ...
    def fetch_history(self, days: int = 365) -> pd.DataFrame:
        ...
        try:
            ...
            df = yf.download(
                tickers_list, period=f"{days}d", interval="1d", progress=False
            )["Close"]
            ...
            logger.error(
                f"Failed to fetch macro history: {e}. Falling back to simulation."
            )
            return self._d_simulate_history(days)
    ...
    def _simulate_price(self, name: str) -> float:
        # Reasonable defaults
        defaults = {"SP500": 5800.0, "VIX": 15.0, "DXY": 104.0, "US10Y": 4.2}
    ...
    def _d_simulate_history(self, days: int) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            {"SP500": sp500, "VIX": vix, "DXY": dxy, "US10Y": us10y}, index=dates
        )
