...
class ModelSelector:
    ...
    def evaluate_and_promote(
        self, candidate_name: str, verification_data: pd.DataFrame
    ):
        """
        ...
        if prod_metrics is None:
            ...
            # Compare PF and DD
            pf_gain = cand_metrics.get("profit_factor", 0) - prod_metrics.get(
                "profit_factor", 0
            )
            dd_red = prod_metrics.get("max_drawdown", 0) - cand_metrics.get(
                "max_drawdown", 0
            )  # e.g. -10 - (-5) = -5 (worse) wait.
            # DD is usually negative.
            # Prod -10%, Cand -5%.
            # We want Cand > Prod (closer to 0).
            # So Cand(-5) > Prod(-10).
            improved_dd = cand_metrics.get("max_drawdown", -1.0) > prod_metrics.get(
                "max_drawdown", -1.0
            )
            improved_pf = cand_metrics.get("profit_factor", 0) > prod_metrics.get(
                "profit_factor", 0
            )

            logger.info(
                f"Comparison: PF({cand_metrics.get('profit_factor'):.2f} vs {prod_metrics.get('profit_factor'):.2f}) | DD({cand_metrics.get('max_drawdown'):.2f} vs {prod_metrics.get('max_drawdown'):.2f})"
            )
