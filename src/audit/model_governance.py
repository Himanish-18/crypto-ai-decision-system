...
def run_audit():
    ...
    df_logs = load_logs()
    df_train = (
        pd.read_parquet(features_path) if features_path.exists() else pd.DataFrame()
    )
    ...
    if "prediction_prob" in df_logs.columns and "action" in df_logs.columns:
        ...
        if "price" in df_logs.columns:
            ...
            prob_true, prob_pred = calibration_curve(
                df_logs["target"][:-1], df_logs["prediction_prob"][:-1], n_bins=10
            )
            ...
            plt.close()
            identity["calibration_score"] = np.mean(
    ...
    if top_features:
        ...
        sns.barplot(
            x=[x[1] for x in sorted_feats],
            y=[x[0] for x in sorted_feats],
            palette="viridis",
        )
        ...
        plt.savefig(
            DASHBOARDS_DIR / "drift_heatmap.png"
        )  # Saving barplot as requested heatmap alternative for single time slice
        plt.close()
