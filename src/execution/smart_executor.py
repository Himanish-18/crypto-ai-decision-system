...
class SmartExecutor:
    ...
    async def execute_order(
        ...
        if microstructure:
            ...
            if impact_cost > 0.0015:  # 0.15%
                logger.warning(
                    f"🛑 High Impact Cost ({impact_cost*100:.3f}%). Aborting Trade."
                )
                return None
        ...
        if style == "AGGRESSIVE":
            ...
            return self.executor.place_order(
                symbol, side, amount, order_type="market", params=params
            )

        elif style == "PASSIVE":
            return await self._execute_passive(
                symbol, side, amount, price
            )  # Passive ignores stops for now
    ...
    async def _execute_passive(
        ...
        order = self.executor.place_order(
            symbol, side, amount, order_type="limit", price=price
        )
        if not order:
        ...
        logger.info(
            f"⏳ Passive Order {order_id} placed at {price}. Est. Queue: {initial_volume_ahead:.4f}"
        )
        ...
        while True:
            ...
            if time.time() - start_time > self.max_wait_time:
                logger.info(
                    f"⏰ Timeout ({self.max_wait_time}s). Switching to AGGRESSIVE."
                )
