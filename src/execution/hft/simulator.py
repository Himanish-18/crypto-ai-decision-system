...
class ExecutionSimulator:
    ...
    def run_simulation(self, snapshots: List[Dict], orders: List[Dict]) -> Dict:
        ...
        for order in orders:
            ...
            for snap in snapshots:
                ...
                if filled:
                    ...
                    fills.append({"order": order, "fill_price": fill_price, "time": fill_time})
                    
                    # Calc Slippage vs Mid at arrival (assuming first snap is arrival)
                    mid_arrival = (snapshots[0]["bids"][0][0] + snapshots[0]["asks"][0][0]) / 2
                    slip = (mid_arrival - fill_price) if side == "SELL" else (fill_price - mid_arrival)
        ...
        return {
            "fill_rate": len([f for f in fills if f["fill_price"]])/len(orders) if orders else 0,
