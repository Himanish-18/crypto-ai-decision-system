...
class InventoryManager:
    ...
    def calculate_target_size(
        self, signal_strength: float, regime_multiplier: float = 1.0
    ) -> float:
        """
        ...
        if self.unrealized_pnl < 0:
            ...
            logger.info(
                f"📉 Losing Position (PnL: {pnl_pct*100:.2f}%). Skew Factor: {skew_factor:.2f}"
            )
            size *= skew_factor
            ...
            logger.info(
                f"📈 Winning Position (PnL: {pnl_pct*100:.2f}%). Skew Factor: {skew_factor:.2f}"
            )
            size *= skew_factor
