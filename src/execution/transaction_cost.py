...
class TransactionCostModel:
    ...
    def estimate_cost(
        ...
        slippage_cost = 0.0
        if style == "AGGRESSIVE":
            params = self.slippage_params.get(
                symbol, {"alpha": 0.5, "daily_vol": 100_000_000}
            )
        ...
        logger.debug(
            f"💰 Cost Est for {symbol} ({style}): Fee=${fee_cost:.4f}, Slippage=${slippage_cost:.4f}, Total=${total_cost:.4f}"
        )
        return total_cost
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Passive Cost (1 BTC): ${cost_passive:.4f} (Should be negative/rebate)"
    )

    # Test Aggressive (Fee + Slippage)
    cost_aggressive = tcm.estimate_cost(
        "BTC/USDT", "buy", 1.0, 50000, style="AGGRESSIVE"
    )
