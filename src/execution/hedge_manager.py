...
class HedgeManager:
    ...
    def check_hedge_requirement(self, portfolio_exposure: float, market_regime: str, btc_prices: list, eth_prices: list, iv_critical: bool = False) -> dict:
        """
        ...
        if iv_critical:
            logger.warning("🛡️ Hedge Manager: CRITICAL IV DETECTED. Forcing Delta Neutrality.")
        ...
        try:
            ...
            beta = self.calculate_hedge_ratio(s_eth, s_btc) # Beta of ETH relative to BTC
            
            logger.info(f"🛡️ Hedge Manager: Regime {market_regime}. Beta(ETH|BTC): {beta:.2f}")
            
            return {
                "action": "HEDGE_SHORT_ETH",
                "ratio": 0.5 # Hedge 50% of exposure
            }
