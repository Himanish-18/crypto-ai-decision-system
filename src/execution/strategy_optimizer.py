...
class StrategyOptimizer:
    ...
    def prepare_data(self):
        ...
        if sf_path.exists():
            ...
            if missing:
                logger.warning(
                    f"⚠️ Missing features in optimizer data: {missing[:5]}..."
                )
                for m in missing:
                    self.test_df[m] = 0.0
        else:
        ...
        if hasattr(self.model, "predict_composite_score"):
            ...
            self.test_df["y_prob"] = self.model.predict_composite_score(
                self.test_df
            ).values
        else:
        ...
        logger.info(
            "Skipping model save in StrategyOptimizer to preserve production model."
        )
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        ...
        sortino = (
            (returns.mean() / downside_returns.std()) * np.sqrt(24 * 365)
...
def main():
    ...
    parser.add_argument(
        "--model_path", type=str, default=str(MODELS_DIR / "best_model_xgb_opt.pkl")
    )
    parser.add_argument(
        "--scaler_path", type=str, default=str(MODELS_DIR / "scaler_opt.pkl")
    )
