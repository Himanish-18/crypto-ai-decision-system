...
class LiveSignalEngine:
    def __init__(self, model_path: Path, scaler_path: Path, balanced_mode: bool = True):
        ...
        self.noise_filter = NoiseFilter(
            window=24, fft_threshold=0.20, var_threshold=0.0005
        )
        self.kalman = KalmanSmoother(process_noise=0.005, measurement_noise=0.03)
        self.vol_adaptive = VolAdaptiveThreshold(
            base_threshold=0.55, vol_sensitivity=0.15
        )
        ...
        self.portfolio_agent = PortfolioPPOAgent(
            input_dim=20, action_dim=5
        )  # Init Agent
    ...
    def load_hybrid_models(self):
        ...
        try:
            ...
            if self.use_onnx and (root / "tiny_cnn_v2.onnx").exists():
                self.tiny_cnn_ort = self.ort.InferenceSession(
                    str(root / "tiny_cnn_v2.onnx")
                )
            ...
            logger.warning(
                f"⚠️ Failed to load Hybrid v5 Models: {e}. Falling back to standard."
            )
    ...
    def load_v19_models(self):
        ...
        try:
            ...
            if (root / "model_failure.pkl").exists():
                self.v19_failure_detector = ModelFailureDetector.load(
                    root / "model_failure.pkl"
                )
    ...
    def load_artifacts(self):
        ...
        self.features_json_path = (
            self.model_path.parent / "selected_alpha_features.json"
        )
        self.selected_features = None

        if self.features_json_path.exists():
            logger.info(
                f"📥 Loading feature selection mask from {self.features_json_path}..."
            )
            import json
            ...
            logger.info(
                f"✅ Feature Mask Loaded: {len(self.selected_features)} features."
            )
    ...
    def process_candle(self, candle_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if self.last_pred_score is not None and self.last_pred_price is not None:
            ret_since_last = (
                current_price - self.last_pred_price
            ) / self.last_pred_price

            # Interpretation (Noise Threshold 0.15%)
            pred_dir = (
                "Bullish"
                if self.last_pred_score > 0.55
                else "Bearish" if self.last_pred_score < 0.45 else "Neutral"
            )
            real_dir = (
                "Up"
                if ret_since_last > 0.0015
                else "Down" if ret_since_last < -0.0015 else "Flat"
            )
        ...
        logger.info(
            f"🔮 Meta-Regime: {meta_regime['predicted_regime']} ({meta_regime['confidence']:.2f})"
        )
        ...
        if self.balanced_mode:
            ...
            if self.tiny_cnn:
                cnn_score = self.tiny_cnn.predict_score(candle_data)
            if self.tcn_lite:
                tcn_score = self.tcn_lite.predict_trend(candle_data.iloc[[-1]])
            ...
            if hasattr(self, "xgb_stacker") and self.xgb_stacker:
                ...
                try:
                    stack_cols = [
                        "btc_alpha_of_imbalance",
                        "btc_alpha_vwap_zscore",
                        "btc_alpha_smart_money_delta",
                    ...
                    X_stack = pd.DataFrame(
                        [vals], columns=stack_cols + ["cnn_prob", "tcn_prob"]
                    )
                    stacker_prob = self.xgb_stacker.predict_proba(X_stack)[0, 1]
                    logger.info(
                        f"🧠 Hybrid v5 Stacker: {stacker_prob:.4f} (CNN={cnn_score:.2f}, TCN={tcn_score:.2f})"
                    )
                    prob = stacker_prob
            ...
            if self.v19_stacker:
                try:
                    ...
                    X_v19 = pd.DataFrame(
                        [[cnn_score, tcn_score]], columns=["xgb_prob", "lgbm_prob"]
                    )
                    ...
                    if self.v19_failure_detector:
                        # Context: Volatility
                        vol_feat = volatility
                        fail_prob = self.v19_failure_detector.predict_failure_prob(
                            {"volatility": vol_feat}
                        )
                        ...
                        if fail_prob > 0.25:
                            logger.warning(
                                f"⛔ v19 Failure Guard Veto: P(Fail) {fail_prob:.2f} > 0.25"
                            )
                            # Update state so log doesn't freeze
                            self.last_pred_score = float(prob)
            ...
            if pem_out["exit_signal"]:
                logger.warning(
                    f"🚨 PANIC EXIT TRIGGERED! Score: {panic_score:.2f} | Reason: {pem_out['reason']}"
                )
                self.last_pred_score = float(prob)
                self.last_pred_price = float(candle_data["btc_close"].iloc[-1])
                return {
            ...
            if "fundingRate" in candle_data.columns:
                ...
                if self.last_funding_rate is not None:
                    ...
                    if np.sign(current_fund) != np.sign(self.last_funding_rate):
                        logger.warning(
                            f"💸 Funding Rate Flip ({self.last_funding_rate} -> {current_fund}). Disabling Longs for 6 candles."
                        )
                        ...
                        self.longs_disabled_until = current_ts + pd.Timedelta(
                            minutes=30
                        )  # 6 * 5m
            ...
            if (
                isinstance(self.longs_disabled_until, int)
                and self.longs_disabled_until == 0
            ):
                self.longs_disabled_until = pd.Timestamp.min.replace(
            ...
            if is_chop:
                logger.warning(
                    "🌪️ Noise Filter: CHOP DETECTED. Requiring higher confidence (0.65)."
                )
            ...
            try:
                # 1h Return
                ret_1h = (
                    candle_data["close"].pct_change(12).iloc[-1]
                    if len(candle_data) >= 12
                    else 0.0
                )  # 12 * 5m = 1h
                # 4h Return
                ret_4h = (
                    candle_data["close"].pct_change(48).iloc[-1]
                    if len(candle_data) >= 48
                    else 0.0
                )
                # Skew (20 period)
                skew = (
                    candle_data["close"].rolling(20).skew().iloc[-1]
                ...
                funding_flip = (
                    ...
                    if (
                        ...
                        and "fundingRate" in candle_data.columns
                        and np.sign(candle_data["fundingRate"].iloc[-1])
                        != np.sign(self.last_funding_rate)
                    )
                    else 0
                )
                ...
                if veto:
                    logger.warning(
                        f"🛡️ LossGuard VETO! P(Loss) {loss_prob:.2f} > 0.6. Blocking Trade."
                    )
                    self.last_pred_score = float(smooth_score)
                    self.last_pred_price = float(candle_data["btc_close"].iloc[-1])
                    return {
            ...
            corr_scalar, hedge_req, corr_debug = (
                self.corr_guard.calculate_risk_modifiers(current_pos_size=tcn_score)
            )
            ...
            try:
                ...
                vol_metric = (
                    candle_data["btc_atr_14"].iloc[-1]
                    / candle_data["btc_close"].iloc[-1]
                    if "btc_atr_14" in candle_data.columns
                    else 0.01
                )
                ...
                if sentiment_score < -0.5 and vol_metric > 0.005:  # 0.5% ATR ~ High Vol
                    logger.warning(
                        f"📰 News Risk: Sentiment {sentiment_score:.2f} (Bearish) + Vol {vol_metric:.4f}. Blocking Longs."
                    )
                    # Return Block Signal
                    self.last_pred_score = float(smooth_score)
                    self.last_pred_price = float(candle_data["btc_close"].iloc[-1])
                    ...
                    logger.info(
                        f"📰 News Sentiment: {sentiment_score:.2f} (Bearish). Caution advised."
                    )
            ...
            if self.dqn_mini:
                ...
                dqn_q = self.dqn_mini.predict_q_value(
                    candle_data.iloc[-1], smooth_score, cnn_score, tcn_score
                )
                ...
                if dqn_q <= 0.0:
                    logger.info(
                        f"⛔ DQN VETO: Q-Value {dqn_q:.6f} <= 0. Signal Blocked."
                    )
                    self.last_pred_score = float(smooth_score)
                    self.last_pred_price = float(candle_data["btc_close"].iloc[-1])
                    return {
        ...
        if volatility < 0.00045:
            logger.info(
                f"🦀 Sideways Micro-Scalp Mode Active (Vol {volatility:.6f} < 0.00045)"
            )
            is_micro_scalp = True
            # Override Chop Block?
        ...
        logger.info(
            f"⚡ SOR Decision: {execution_mode} (Urg={urgency}, Vol={volatility:.4f})"
        )
        ...
        if not is_micro_scalp:  # Only apply standard TD checks if not in micro mode
            if td_score > 0.65:
                ...
                logger.info(
                    f"📉 Trend Boost: Threshold lowered to {effective_threshold:.4f}"
                )
                ...
                logger.info(
                    f"🛡️ Weak Trend Protection: Threshold raised to {effective_threshold:.4f}"
                )
                execution_mode = "MAKER"
        ...
        if is_chop:
            ...
            if smooth_score > effective_threshold:
                ...
                logger.info(
                    f"💤 Score {smooth_score:.4f} <= Eff. Threshold {effective_threshold:.4f}"
                )
        ...
        # v10 TCM Cost Check
        if final_signal == 1:
            est_cost = self.tcm.estimate_cost(
                "buy", 1000, volatility, 0.0001, mode=execution_mode
            )  # Dummy spread
            if not self.tcm.is_profitable(
                0.002, est_cost
            ):  # Require 0.2% raw return? Heuristic.
                logger.warning(
                    f"💸 TCM VETO: Expected Return < Cost ({est_cost*100:.3f}%). Blocked."
                )
                final_signal = 0
        ...
        if final_signal == 1:
            ...
            logger.info(
                f"🧠 PPO Sizing: {size_scalar:.2f}x (Prob={smooth_score:.2f}, Vol={volatility:.4f})"
            )

            # If PPO says 0 size, we block
            if size_scalar < 0.1:
                logger.warning(
                    f"🧠 PPO Veto: Size {size_scalar:.2f} < 0.1. Signal Blocked."
                )
                final_signal = 0
        ...
        if final_signal == 1:
            logger.info(
                f"✅ SIGNAL GENERATED: Score {smooth_score:.4f} | Size: {size_scalar:.2f}x | Exec: {execution_mode}"
            )
            self.last_pred_score = float(smooth_score)
            self.last_pred_price = float(candle_data["btc_close"].iloc[-1])
            return {
