...
class ExecutionEngineV3:
    ...
    def __init__(self):
        ...
        if os.path.exists("data/fills.csv") and os.path.getsize("data/fills.csv") == 0:
            fill_logger.info("timestamp,action,size,type,price,exec_inst,fill_price,slippage")
    ...
    def execute_order(self, decision: Dict[str, Any], market_meta: Dict[str, Any]):
        ...
        logger.info(f"🔫 Executing {action} {size} | Mode: {mode} | Plan: {route_plan.get('exec_inst', 'MARKET')}")
    ...
    def _execute_taker(self, action: str, size: float, meta: Dict):
        ...
        fill_price = price * (1 + slippage) if action == "BUY" else price * (1 - slippage)
    ...
    def _log_fill(self, side, size, type, price, fill_price):
         fill_logger.info(f"{time.time()},{side},{size},{type},{price},NORMAL,{fill_price},{abs(fill_price-price)}")
