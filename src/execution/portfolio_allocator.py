...
class PortfolioAllocator:
    ...
    def set_target_weights(self, weights: Dict[str, float]):
        ...
        if not abs(sum(weights.values()) - 1.0) < 1e-6:
            logger.warning(f"Weights do not sum to 1.0: {sum(weights.values())}. Normalizing...")
    ...
    def check_exposure_limit(self, symbol: str, current_exposure: float, new_trade_size: float) -> bool:
        """
        ...
        if current_exposure + new_trade_size > max_alloc:
            logger.warning(f"🚫 Exposure Limit Breach for {symbol}. Max: {max_alloc}, Current: {current_exposure}, New: {new_trade_size}")
            return False
        return True
...
if __name__ == "__main__":
    ...
    # Check Limit
    is_ok = allocator.check_exposure_limit("BTC/USDT", current_exposure=4000, new_trade_size=1500)
    logger.info(f"Trade Allowed (4000+1500 <= 5000)? {is_ok}")
    
    is_ok = allocator.check_exposure_limit("BTC/USDT", current_exposure=4000, new_trade_size=500)
