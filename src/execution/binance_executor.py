...
class BinanceExecutor:
    def __init__(self):
        ...
        if not self.api_key or not self.secret_key:
            ...
            if self.testnet:
                ...
                logger.warning(
                    "🚨 Connected to Binance MAINNET 🔴 (REAL MONEY AT RISK)"
                )
    ...
    def _initialize_safety_locks(self):
        ...
        try:
            ...
            logger.info(
                f"🔒 Safety Lock Initialized. Start Balance: ${self.initial_balance:.2f} USDT"
            )
    ...
    def _check_safety_status(self):
        ...
        if drawdown_pct >= self.kill_switch_drop:
            logger.critical(
                f"💀 MISSING KILL SWITCH! Drawdown {drawdown_pct*100:.2f}% >= {self.kill_switch_drop*100}%"
            )
        ...
        if drawdown_pct >= self.max_daily_loss:
            logger.warning(
                f"🛑 Max Daily Loss Hit ({drawdown_pct*100:.2f}%). Trading Halted."
            )
            return False
    ...
    def execute_order(
        ...
        if not self.exchange:
            logger.info(
                f"DRY-RUN: Executing {side.upper()} {amount} {symbol} @ {order_type}"
            )
