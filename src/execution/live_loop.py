...
class LiveBotService:
    ...
    def job(self):
        ...
        try:
            ...
            if not self.guardian.check_system_health(self.signal_engine.model, self.signal_engine.scaler, df):
            ...
            if current_balance is None:
                logger.warning("⚠️ Failed to fetch balance. Skipping cycle to avoid false alarms.")
                return
            ...
            if not self.executor.testnet and current_balance < 100:
                logger.critical(f"🛑 Go-Live Safety: Balance ${current_balance} < $100. Aborting.")
                return
            ...
            if not self.guardian.check_exposure(current_exposure, current_balance):
                 logger.warning(f"🛑 Guardian: Exposure Limit Reached (Exp: {current_exposure}). Skipping Trade.")
                 return
            ...
            if not self.guardian.check_market_conditions(signal_output["strategy_context"]):
            ...
            if decision["action"] == "BUY":
                ...
                if current_balance > 10:
                    ...
                    if not self.guardian.check_execution_safety(current_price, current_price):
            ...
            logger.info(f"📝 Paper Trade Logged: {decision['action']} | Size: {decision.get('size', 0.0)}")
