...
def add_lagged_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for symbol in ["btc", "eth"]:
        ...
        df[f"{symbol}_ret"] = np.log(
            df[f"{symbol}_close"] / df[f"{symbol}_close"].shift(1)
        )
...
def engineer_sentiment(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for w in windows:
        ...
        sent_mass = (
            (df["sentiment_mean"] * df["sentiment_count"]).rolling(window=w).sum()
        )
