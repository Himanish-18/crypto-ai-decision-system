...
class AlphaSignals:
    ...
    def _compute_formulaic(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        df[f"{symbol}_alpha_009"] = np.where(cond1, delta_c, np.where(cond2, delta_c, -delta_c))
    ...
    def _compute_microstructure(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        df[f"{symbol}_alpha_vol_imbalance"] = (up_vol - down_vol) / (up_vol + down_vol + 1e-9)
    ...
    def _compute_technical_alphas(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        width = (bb.bollinger_hband() - bb.bollinger_lband()) / (bb.bollinger_mavg() + 1e-9)
    ...
    def _compute_v5_alphas(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        of_imbalance = (direction * v).rolling(window=10).sum() / (v.rolling(window=10).sum() + 1e-9)
        ...
        rolling_vwap = (v * (h+l+c)/3).rolling(24).sum() / (v.rolling(24).sum() + 1e-9)
        ...
        df[f"{symbol}_alpha_gap_strength"] = (o - c.shift(1)) / (c.rolling(24).std() + 1e-9)
        ...
        decay = np.where((roc > 0) & (hist.diff() < 0), 1, 
