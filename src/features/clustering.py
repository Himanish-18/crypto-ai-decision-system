...
class FeatureClustering:
    ...
    def fit(self, X: pd.DataFrame):
        ...
        linkage_matrix = hierarchy.watch_linkage = hierarchy.linkage(dist_linkage, method='ward')
        ...
        cluster_labels = hierarchy.fcluster(linkage_matrix, t=(1 - self.threshold), criterion='distance')
        ...
        logger.info(f"Feature Clustering: Reduced {len(X.columns)} -> {len(self.selected_features)} features.")
        logger.info(f"Dropped {len(X.columns) - len(self.selected_features)} redundant features.")
