...
class SentimentFeatures:
    ...
    def calculate_proxies(df: pd.DataFrame, window: int = 30) -> pd.DataFrame:
        ...
        if "feat_synthetic_sentiment" not in df.columns:
            if "openInterest" not in df.columns:
                ...
                df["feat_sentiment_proxy"] = (
                    df[close_col].rolling(window).corr(oi_proxy)
                )
            else:
                if "btc_close" in df.columns:
                    close_col = "btc_close"
                    ...
                    close_col = "close"
                df["feat_sentiment_proxy"] = (
                    df[close_col].rolling(window).corr(df["openInterest"])
                )
        ...
        if (
            high_col not in df.columns
            or low_col not in df.columns
            or close_col not in df.columns
            or vol_col is None
        ):
    ...
    @staticmethod
    def calculate_synthetic_sentiment(
        df: pd.DataFrame, window: int = 24
    ) -> pd.DataFrame:
        """
        ...
        if (
            "feat_sentiment_proxy" not in df.columns
            or df["feat_sentiment_proxy"].isna().all()
        ):
