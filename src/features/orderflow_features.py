...
class OrderFlowFeatures:
    ...
    def _compute_imbalance_proxy(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        df[f"{symbol}_of_flow_24h"] = (
            df[f"{symbol}_of_buy_sell_imbalance"].rolling(24).sum()
        )
    ...
    def _compute_wick_metrics(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        df[f"{symbol}_of_wick_reversal"] = (
            np.maximum(upper_wick, lower_wick) / total_range
        )
    ...
    def _compute_volatility_proxies(
