...
class FormulaicAlphas:
    ...
    def compute_all(self, df: pd.DataFrame, symbol: str = "btc") -> pd.DataFrame:
        ...
        df[f"{symbol}_alpha_009"] = np.where(cond1, delta_close, np.where(cond2, delta_close, -delta_close))
        ...
        df[f"{symbol}_alpha_018"] = -1 * (term - term.rolling(20).mean()) / (term.rolling(20).std() + 1e-9)
        ...
        cond = ts_corr(vwap, ts_delay(close, 5), 20) < 0 # Reduced window 230->20 for 1H crypto
        df[f"{symbol}_alpha_021"] = (close.rolling(7).mean() - close) + np.where(cond, close, 0)
        
        # Alpha#26: (-1 * ts_max(corr(ts_rank(volume, 5), ts_rank(high, 5), 5), 3))
        df[f"{symbol}_alpha_026"] = -1 * ts_max(ts_corr(ts_rank(volume, 5), ts_rank(high, 5), 5), 3)
        ...
        df[f"{symbol}_alpha_034"] = 1 - (ret.rolling(2).std() / (ret.rolling(5).std() + 1e-9)).rolling(10).rank(pct=True)
