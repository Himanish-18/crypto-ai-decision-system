...
def load_logs():
    ...
    target_log = (
        LOG_FILE if LOG_FILE.exists() and LOG_FILE.stat().st_size > 0 else PAPER_LOG
    )
...
if not df.empty:
    ...
    # Signal Confidence
    fig_conf = px.line(
        df, x="timestamp", y="signal_confidence", title="Signal Confidence"
    )
    ...
    if "regime" in df.columns:
        fig_regime = px.scatter(
            df, x="timestamp", y="btc_close", color="regime", title="Regime Detection"
        )
...
with col_d2:
    ...
    if "prediction_prob" in df.columns:
        fig_hist = px.histogram(
            df, x="prediction_prob", nbins=20, title="Model Confidence"
        )
        fig_hist.add_vline(
            x=0.55, line_dash="dash", line_color="green", annotation_text="Threshold"
        )
...
st.caption(
    "Live metrics should ideally outperform 'Base Case' and avoid 'Critical' scenarios."
)
