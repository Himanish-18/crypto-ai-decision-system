...
def train_expanded_model():
    ...
    try:
        if "normal" in mf_model.models:
            ...
            if "xgb" in ensemble.models:
                ...
                if hasattr(ensemble.scaler, "feature_names_in_"):
                    ...
                    feature_names = [c for c in df.columns if c not in exclude and pd.api.types.is_numeric_dtype(df[c])]
                ...
                logger.info(f"Selected {len(selected_features)} features for live inference.")
        ...
        exclude = ["timestamp", "y_direction_up", "regime", "symbol", "btc_ret_fwd_1"]
        selected_features = [c for c in df.columns if c not in exclude and pd.api.types.is_numeric_dtype(df[c])]
