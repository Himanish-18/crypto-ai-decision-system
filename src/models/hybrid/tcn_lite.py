...
class TCNLiteProxy:
    ...
    def fit(self, df: pd.DataFrame, target_col="target"):
        ...
        candidates = [c for c in df.columns if c not in exclude and np.issubdtype(df[c].dtype, np.number)]
        
        # 2. Strict Filter: Remove Targets and Future Leaks
        # Remove anything with 'fwd', 'target', 'y_', 'future'
        candidates = [c for c in candidates if not any(x in c for x in ['fwd', 'target', 'y_', 'future'])]
