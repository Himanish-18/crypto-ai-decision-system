...
class DQNMiniProxy:
    ...
    def prepare_state(self, row, mf_score, cnn_score, tcn_score):
        ...
        regime_code = 0.0
        if regime == "Trend": regime_code = 1.0
        elif regime == "High Volatility": regime_code = 2.0
        elif regime == "Low Liquidity": regime_code = 3.0
        
        vol = row.get("btc_atr_14", 0) / (row.get("btc_close", 1) + 1e-9)
        
        return np.array([mf_score, cnn_score, tcn_score, regime_code, vol]).reshape(1, -1)
