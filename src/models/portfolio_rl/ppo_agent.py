...
class RiskWeightedMemory:
    ...
    def push(self, state, action, reward, next_state, done, profit_factor, confidence, log_prob):
        """
...
class PortfolioActorCritic(nn.Module):
    ...
    def evaluate(self, state, action):
        ...
        dist = torch.distributions.Normal(action_mean, 0.1) # Fixed std dev for exploration
...
class PortfolioPPOAgent:
    ...
    def update_policy(self):
        ...
        for _ in range(self.K_epochs):
            ...
            advantages = rewards - state_values.detach() # Simplified advantage (Reward - Baseline)
        ...
        for i, p in enumerate(self.model.parameters()):
            drift = torch.norm(p - self.initial_params[i]) / torch.norm(self.initial_params[i])
            if drift > 0.01: # 1% threshold
                logger.warning(f"⚠️ Parameter drift {drift:.4f} > 1%. Reverting this update.")
