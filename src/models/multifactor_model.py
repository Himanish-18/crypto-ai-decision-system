...
class MultiFactorModel:
    ...
    def __init__(self):
        ...
        self.weights = {"rank_agg": 0.3, "weighted": 0.2, "stacking": 0.5}
    ...
    def train(self, df: pd.DataFrame, target_col="y_direction_up"):
        ...
        if not df_crisis.empty:
            self.models["crisis"] = (
                AlphaEnsemble()
            )  # Could customize params here for robustness (e.g. less depth)
            ...
            if "normal" in self.models:
                ...
                self.models["crisis"] = (
                    AlphaEnsemble()
                )  # Initialize an empty model or handle gracefully
    ...
    def predict_composite_score(self, df: pd.DataFrame) -> pd.Series:
        ...
        score_weighted = (
            df[alpha_cols]
            .apply(lambda x: (x - x.mean()) / (x.std() + 1e-9), axis=0)
            .mean(axis=1)
        )
        ...
        if len(idx_normal) > 0:
            if "normal" in self.models:
                ...
                logger.warning(
                    "Normal model not trained. Defaulting to 0.5 for normal regime samples."
                )
        ...
        if len(idx_crisis) > 0:
            if "crisis" in self.models:
                ...
                "normal" in self.models
            ):  # Fallback if crisis model not explicitly trained but normal is
                logger.info(
                    "Crisis model not trained, falling back to Normal model for crisis regime samples."
                )
                subset = df.loc[idx_crisis]
                pred = self.models["normal"].predict_proba(subset)
                scores.loc[idx_crisis] = pred
                ...
                logger.warning(
                    "Neither Crisis nor Normal model trained. Defaulting to 0.5 for crisis regime samples."
                )
                scores.loc[idx_crisis] = 0.5

        score_stacking = scores.fillna(
            0.5
        )  # Fill any remaining NaNs (shouldn't happen if all paths covered)
