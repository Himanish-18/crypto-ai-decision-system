...
class RegimeDetector:
    def __init__(self, n_components: int = 3, covariance_type: str = "full", n_iter: int = 100):
    ...
    def _map_states(self, X_scaled):
        ...
        for i in range(self.n_components):
            logger.info(f"State {i}: Vol={means[i,0]:.4f}, Ret={means[i,1]:.4f}, Mom={means[i,2]:.4f}, Skew={means[i,3]:.4f} -> {self.state_map[i]}")
