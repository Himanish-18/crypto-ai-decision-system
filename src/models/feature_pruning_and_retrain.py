...
def get_initial_features(df):
    exclude_cols = ["timestamp", TARGET_COL, "btc_ret_fwd_1", "y_pred", "y_prob", "signal_prob", "regime", "symbol"]
...
def correlation_pruning(df, feature_cols, threshold=0.85):
    ...
    logger.info(f"Dropped {len(to_drop)} features due to correlation > {threshold}. Remaining: {len(selected)}")
    return selected
...
def shap_pruning(df, feature_cols, target, threshold=0.001):
    ...
    model = xgb.XGBClassifier(n_estimators=100, max_depth=3, learning_rate=0.05, n_jobs=-1, random_state=42)
    model.fit(X, y)
    ...
    feature_importance = pd.DataFrame(list(zip(feature_cols, shap_abs)), columns=['feature', 'shap_value'])
...
def run_optimization(df, feature_cols):
    ...
    study_xgb.optimize(lambda trial: objective_xgb(trial, X_scaled, y), n_trials=20) # 20 trials for speed
...
def train_final_ensemble(df, feature_cols, best_xgb_params, best_lgb_params, scaler):
    ...
    for train_idx, val_idx in tscv.split(X_scaled):
        ...
        # LGB
        lgb_m = lgb.LGBMClassifier(**best_lgb_params, verbose=-1, n_jobs=-1, random_state=42)
        ...
        meta_df = pd.DataFrame({"xgb": p_xgb, "lgb": p_lgb, "lr": p_lr}, index=X_val.index)
    ...
    ensemble.models["xgb"] = xgb.XGBClassifier(**best_xgb_params, n_jobs=-1, random_state=42).fit(X_scaled, y)
    ensemble.models["lgb"] = lgb.LGBMClassifier(**best_lgb_params, verbose=-1, n_jobs=-1, random_state=42).fit(X_scaled, y)
...
def main():
    ...
    # 3. Final Training
    ensemble, final_auc, coefs = train_final_ensemble(df, final_vars, best_xgb, best_lgb, scaler)
