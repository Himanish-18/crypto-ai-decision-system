...
def prepare_features(
    df: pd.DataFrame, target_col: str = "y_direction_up"
...
def run_optimization(X, y):
    ...
    study_xgb.optimize(
        lambda trial: objective_xgb(trial, X, y, tscv), n_trials=10
    )  # Reduced trials for speed
    logger.info(
        f"Best XGB Params: {study_xgb.best_params} | Score: {study_xgb.best_value:.4f}"
    )
    ...
    logger.info(
        f"Best LGB Params: {study_lgb.best_params} | Score: {study_lgb.best_value:.4f}"
    )
...
def feature_importance_analysis(model, X_test):
    ...
    feature_importance = pd.DataFrame(
        list(zip(X_test.columns, vals)), columns=["col_name", "feature_importance_vals"]
    )
...
def main():
    ...
    # 3. Train Best Models
    xgb_model, lgb_model, X_test, y_test = train_best_models(
        X, y, xgb_params, lgb_params
    )
    ...
    logger.info(
        f"✅ Model Improvement Pipeline Complete. Saved to {best_model_path} and {scaler_path}"
    )
