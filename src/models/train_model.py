...
def split_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    logger.info(
        f"✂️ Data Split: Train={len(train_df)}, Val={len(val_df)}, Test={len(test_df)}"
    )
...
def prepare_features(
    ...
    exclude_cols = [
        ...
        target_col,
        "btc_ret_fwd_1",
    ]  # btc_ret_fwd_1 is the raw target, y_direction_up is the class
...
def train_lstm(
    ...
    train_data = TensorDataset(
        torch.from_numpy(X_train_seq).float(), torch.from_numpy(y_train_seq).float()
    )
    val_data = TensorDataset(
        torch.from_numpy(X_val_seq).float(), torch.from_numpy(y_val_seq).float()
    )

    train_loader = DataLoader(
        train_data, shuffle=False, batch_size=batch_size
    )  # Shuffle=False for time series? Usually True for batches is fine if sequences are independent samples, but let's keep False to be safe/consistent
