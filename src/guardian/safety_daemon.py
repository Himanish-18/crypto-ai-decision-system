...
class SafetyDaemon:
    ...
    def check_system_health(
        self, model, scaler, features_df: pd.DataFrame, timeframe_minutes: int = 60
    ) -> bool:
        ...
        if delay > max_allowed_delay:
            logger.error(
                f"Guardian: Data Stale! Delay: {delay:.0f}s > {max_allowed_delay}s. PAUSING."
            )
    ...
    def check_market_conditions(self, market_state: dict) -> bool:
        ...
        if atr_pct > self.max_volatility_percentile:
            logger.warning(
                f"Guardian: Extreme Volatility (ATR Pct: {atr_pct:.4f}). NO NEW TRADES."
            )
            return False
    ...
    def check_financial_health(self, current_equity: float) -> bool:
        ...
        if self.state["is_locked"]:
            logger.critical(
                f"Guardian: Account LOCKED due to: {self.state['lock_reason']}"
            )
            return False
        ...
        if total_dd > self.max_total_drawdown_pct:
            ...
            self.state["lock_reason"] = (
                f"Total Drawdown {total_dd*100:.2f}% > {self.max_total_drawdown_pct*100}%"
            )
            self._save_state()
        ...
        if daily_dd > self.max_daily_drawdown_pct:
            logger.error(
                f"Guardian: Daily Drawdown {daily_dd*100:.2f}% > {self.max_daily_drawdown_pct*100}%. TRADING DISABLED FOR TODAY."
            )
            return False
    ...
    def check_trade_streak(self, last_trade_pnl: float):
        ...
        if self.state["losing_streak"] >= self.max_losing_streak:
            self.state["is_locked"] = True
            self.state["lock_reason"] = (
                f"Kill Switch: {self.state['losing_streak']} consecutive losses."
            )
            logger.critical(f"Guardian: {self.state['lock_reason']} LOCKING ACCOUNT.")
    ...
    def check_exposure(self, current_exposure: float, total_capital: float) -> bool:
        ...
        if exposure_pct > self.max_exposure_pct:
            logger.warning(
                f"Guardian: Max Exposure Reached ({exposure_pct*100:.2f}% > {self.max_exposure_pct*100}%). NO NEW TRADES."
            )
            return False
        return True
    ...
    def check_execution_safety(self, current_price: float, order_price: float) -> bool:
        ...
        if diff > self.max_slippage_pct:
            logger.warning(
                f"Guardian: High Slippage Risk! Diff: {diff*100:.2f}%. HOLDING."
            )
            return False
