...
class IVGuard:
    ...
    def check_trade(self, vol_metrics: dict, is_hedged: bool) -> bool:
        ...
        if is_critical:
            if is_hedged:
                logger.info(
                    f"🛡️ IV Guard: Critical Vol ({vol_metrics['current_iv']:.2f}) but HEDGED. Allowing trade."
                )
                return True
            else:
                logger.warning(
                    f"⛔ IV Guard: Critical Vol ({vol_metrics['current_iv']:.2f}) & Unhedged. BLOCKING Trade."
                )
                return False
        ...
        if crash_prem > 0.10:  # 10 vol points skew
            logger.warning(
                f"⛔ IV Guard: Extreme Skew ({crash_prem:.2f}). Blocking Unhedged Directional."
            )
