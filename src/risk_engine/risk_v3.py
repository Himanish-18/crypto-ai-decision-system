...
class RiskEngineV3:
    ...
    def check_trade_risk(
        self, proposed_trade: Dict[str, Any], market_factors: Dict[str, float]
    ) -> bool:
        """
        ...
        if abs(projected_beta) > self.max_beta:
            logger.warning(
                f"🛑 REJECT: Portfolio Beta {projected_beta:.2f} > Limit {self.max_beta}"
            )
            return False
    ...
    def calculate_greeks(
        self, price: float, vol: float, time_to_expiry: float = 30 / 365
    ):
        """
