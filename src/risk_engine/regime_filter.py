...
class RegimeFilter:
    ...
    def fit_predict_and_save(self, df: pd.DataFrame, symbol: str = "btc") -> pd.DataFrame:
        ...
        # Save Labels
        labels_df = pd.DataFrame({"timestamp": df["timestamp"], "regime": final_regimes})
        ...
        for i, row in enumerate(final_regimes):
            ...
            elif a > atr_thresh or row == "High Volatility": # Keep GMM HighVol if reliable
    ...
    def get_risk_params(self, regime: str) -> dict:
        ...
        if regime == "Bull Trend":
            return {"stop_loss": 0.03, "take_profit": 0.10, "position_size": 1.0, "entry_threshold": 0.55}
        elif regime == "Bear Trend":
             return {"stop_loss": 0.02, "take_profit": 0.05, "position_size": 0.5, "entry_threshold": 0.65} # Strict
        elif regime == "Sideways":
             return {"stop_loss": 0.02, "take_profit": 0.03, "position_size": 0.8, "entry_threshold": 0.60} # Mean rev
        elif regime == "High Volatility":
             return {"stop_loss": 0.05, "take_profit": 0.15, "position_size": 0.3, "entry_threshold": 0.70} # Safety
        else:
             return {"stop_loss": 0.02, "take_profit": 0.04, "position_size": 0.5, "entry_threshold": 0.60}
