    ...
    def calculate_risk_modifiers(self, current_pos_size=1.0):
        ...
        if max_corr > 0.6:
            ...
            reduction_factor = max(
                0.0, (max_corr - 0.6) * 1.66
            )  # 0.3 diff -> 0.5 reduction
            size_scalar = max(0.5, 1.0 - reduction_factor)
            logger.info(
                f"🔗 High Correlation ({max_corr:.2f}). Reducing Size Scalar to {size_scalar:.2f}"
            )
        ...
        if avg_corr > 0.7 and current_pos_size > 0.5:
            hedge_signal = True
            logger.warning(
                f"🔗 Systemic Risk (Avg Corr {avg_corr:.2f}). HEDGING SIGNAL ACTIVE."
            )
