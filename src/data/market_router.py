...
class MarketRouter:
    ...
    """
    def __init__(self, primary_exchange: str = "binance", secondary_exchanges: List[str] = ["bybit", "okx", "coinbasepro"]):
    ...
    async def fetch_unified_candles(self, symbol: str = "BTC/USDT", timeframe: str = "1m", limit: int = 100) -> pd.DataFrame:
        """
        Fetch OHLCV from Primary. If fail, switch to Secondary.
        Returns standardized DataFrame.
        """
        # Try Primary
        df = await self._fetch_exchange_candles(self.primary_name, symbol, timeframe, limit)
        if df is not None:
            return df
    ...
    async def _fetch_exchange_candles(self, exchange_name: str, symbol: str, timeframe: str, limit: int) -> Optional[pd.DataFrame]:
    ...
    async def get_aggregated_depth(self, symbol: str = "BTC/USDT", depth_limit: int = 10) -> Dict:
        ...
        for name, exchange in self.exchanges.items():
            tasks.append(asyncio.to_thread(exchange.fetch_order_book, symbol, depth_limit))
