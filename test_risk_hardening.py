...
def test_risk_engine():
    ...
    size_low_vol = risk.calculate_position_size(
        win_rate=0.6, entry_price=100, volatility=0.01
    )
    ...
    size_high_vol = risk.calculate_position_size(
        win_rate=0.6, entry_price=100, volatility=0.15
    )
    logger.info(f"Size (High Vol 15%): {size_high_vol:.2f} units")

    assert (
        size_high_vol < size_low_vol
    ), f"High volatility should reduce position size. Low: {size_low_vol}, High: {size_high_vol}"
